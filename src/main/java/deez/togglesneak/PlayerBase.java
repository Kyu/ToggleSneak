	...
	public void onLivingUpdate()
	{
		...
		*/
			...
			if(this.playerAPI.getInPortalField())
			{
				...
				{
						this.mc.getSoundHandler().playSound(new PositionedSoundRecord(new SoundEvent(new ResourceLocation("portal.trigger")), SoundCategory.AMBIENT, 1.0F, this.player.getRNG().nextFloat() * 0.4F + 0.8F, player.getPosition()));
				}
			...
			else if(this.player.isPotionActive(Potion.getPotionFromResourceLocation("effect.confusion")) && this.player.getActivePotionEffect(Potion.getPotionFromResourceLocation("effect.confusion")).getDuration() > 60)
			{
			...
			*/
			...
			if(isSprintDisabled)
			{
	            if(ToggleSneakMod.optionDoubleTap && this.player.onGround && !isMovingForward && this.player.movementInput.moveForward >= minSpeed && !this.player.isSprinting() && enoughHunger && !this.player.isHandActive() && !this.player.isPotionActive(Potion.getPotionFromResourceLocation("effect.blindness")))
	            {
	            ...
	            if(!this.player.isSprinting() && this.player.movementInput.moveForward >= minSpeed && enoughHunger && !this.player.isHandActive() && !this.player.isPotionActive(Potion.getPotionFromResourceLocation("effect.blindness")) && this.settings.keyBindSprint.isKeyDown())
	            {
	               this.player.setSprinting(true);
	               customMovementInput.UpdateSprint(true, false);
	            }
			...
			else
			{
				...
				//if(this.player.onGround && enoughHunger && !this.player.isHandActive() && !this.player.isPotionActive(Potion.getPotionFromResourceLocation("effect.blindness")) && !this.customMovementInput.sprintHeldAndReleased)

				if(enoughHunger && !this.player.isHandActive() && !this.player.isPotionActive(Potion.getPotionFromResourceLocation("effect.blindness")) && !this.customMovementInput.sprintHeldAndReleased)
				{
	            ...
	            if(canDoubleTap && !state && this.player.onGround && !isMovingForward && this.player.movementInput.moveForward >= minSpeed && !this.player.isSprinting() && enoughHunger && !this.player.isHandActive() && !this.player.isPotionActive(Potion.getPotionFromResourceLocation("effect.blindness")))
	            {
	               if(this.playerAPI.getSprintToggleTimerField() == 0)
	               {
	                  this.playerAPI.setSprintToggleTimerField(7);
